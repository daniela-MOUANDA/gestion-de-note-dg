import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faBell, faUser } from '@fortawesome/free-solid-svg-icons'
import { useAuth } from '../../contexts/AuthContext'

const HeaderChefDepartement = ({ chefName }) => {
  const { user } = useAuth()
  
  // Récupérer le prénom et nom de l'utilisateur
  const userFirstName = user?.prenom || ''
  const userLastName = user?.nom || ''
  const userPhoto = user?.photo || null
  const departementNom = user?.departement?.nom || 'Département'
  
  // Initiales : première lettre du prénom + première lettre du nom
  const initials = userFirstName && userLastName 
    ? `${userFirstName[0]}${userLastName[0]}`.toUpperCase()
    : userFirstName 
      ? userFirstName.substring(0, 2).toUpperCase()
      : 'CD'
  
  // URL de la photo
  const photoUrl = userPhoto 
    ? (userPhoto.startsWith('http') ? userPhoto : `http://localhost:3000${userPhoto}`)
    : null

  const displayName = chefName || `${userFirstName} ${userLastName}` || 'Chef de Département'

  return (
    <header className="fixed top-0 left-0 lg:left-64 right-0 bg-white border-b border-slate-200 shadow-sm z-20">
      <div className="flex items-center justify-between px-4 sm:px-6 lg:px-8 py-4 pb-4 mb-4">
        <div className="flex-1">
          <h2 className="text-lg sm:text-xl font-bold text-slate-800">
            Département {departementNom}
          </h2>
          <p className="text-xs sm:text-sm text-slate-600 mt-1">
            Chef de Département - Gestion pédagogique et académique
          </p>
        </div>
        
        <div className="flex items-center gap-4">
          <button className="relative p-2 text-slate-600 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
            <FontAwesomeIcon icon={faBell} className="text-xl" />
            <span className="absolute top-1 right-1 w-2 h-2 bg-red-500 rounded-full"></span>
          </button>
          
          <div className="flex items-center gap-3 pl-4 border-l border-slate-200">
            <div className="hidden sm:block text-right">
              <p className="text-sm font-semibold text-slate-800">{displayName}</p>
              <p className="text-xs text-slate-600">Chef de Département</p>
            </div>
            <div className="w-10 h-10 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-full flex items-center justify-center overflow-hidden">
              {photoUrl ? (
                <img 
                  src={photoUrl} 
                  alt={displayName}
                  className="w-full h-full object-cover"
                  onError={(e) => {
                    e.target.style.display = 'none'
                    e.target.nextSibling.style.display = 'flex'
                  }}
                />
              ) : null}
              <div className={`w-full h-full flex items-center justify-center ${photoUrl ? 'hidden' : ''}`}>
                <span className="text-white text-xs font-semibold">{initials}</span>
              </div>
            </div>
          </div>
        </div>
      </div>
    </header>
  )
}

export default HeaderChefDepartement

